...
-define(DEFAULT_BUS_SYSTEM, #bus_id{
    scheme = unix, options = [{path, "/var/run/dbus/system_bus_socket"}]
}).
...
%% @end
-spec call({?MODULE, dbus_connection()} | dbus_connection(), dbus_message()) ->
    {ok, term()} | {error, term()}.
...
%% @end
-spec cast({?MODULE, dbus_connection()} | dbus_connection(), dbus_message()) ->
    ok | {error, term()}.
...
%% @end
-spec get_unique_name({?MODULE, dbus_connection()} | dbus_connection()) ->
...
to_bus_id(Server) when is_list(Server) ->
    ...
    #bus_id{
        scheme = list_to_existing_atom(Transport),
