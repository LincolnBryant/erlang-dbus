...
to_xml(#dbus_node{} = Node) ->
    Prolog =
        "<?xml version=\"1.0\" encoding=\"utf-8\" ?><!DOCTYPE node PUBLIC \"-//freedesktop//DTD D-BUS Object Introspection 1.0//EN\" \"http://www.freedesktop.org/standards/dbus/1.0/introspect.dtd\">",
...
to_xmerl(#dbus_iface{} = Elem) ->
    {interface, [{name, to_binary(Elem#dbus_iface.name)}],
...
xml_event(
    {startElement, _, "arg", _, Attrs},
    _L,
    #state{s = method, method = #dbus_method{args = Args} = Method} = S
) ->
...
xml_event(
    {startElement, _, "arg", _, Attrs},
    _L,
    #state{s = signal, signal = #dbus_signal{args = Args} = Signal} = S
) ->
    Arg = build_arg(Attrs, #dbus_arg{}),
    S#state{s = signal_arg, signal = Signal#dbus_signal{args = [Arg | Args]}};
xml_event(
    {startElement, _, "annotation", _, Attrs},
    _L,
    #state{s = method, method = #dbus_method{annotations = Annotations} = M} = S
) ->
    S#state{
        s = method_annotation,
        method = M#dbus_method{
            annotations = [build_annotation(Attrs, {undefined, undefined}) | Annotations]
        }
    };
xml_event(
    {startElement, _, "annotation", _, Attrs},
...
xml_event(
    ...
    S#state{
        s = property_annotation,
        property = P#dbus_property{
            annotations = [build_annotation(Attrs, {undefined, undefined}) | Annotations]
        }
    };
xml_event(
...
xml_event(
    {endElement, _, "node", _},
    _L,
    #state{s = child_node, node = #dbus_node{elements = E} = N, child = C} = S
) ->
    S#state{s = node, node = N#dbus_node{elements = [C | E]}, child = undefined};
xml_event(
    {endElement, _, "interface", _},
    _L,
    #state{s = iface, node = #dbus_node{interfaces = Ifaces} = N, iface = I} = S
) ->
...
xml_event(
    {endElement, _, "method", _},
    _L,
    #state{s = method, iface = #dbus_iface{methods = Methods} = I, method = M} = S
) ->
...
xml_event(
    {endElement, _, "signal", _},
    _L,
    #state{s = signal, iface = #dbus_iface{signals = Signals} = I, signal = Sig} = S
) ->
    NewIface = I#dbus_iface{
        signals = gb_trees:insert(Sig#dbus_signal.name, end_signal(Sig), Signals)
    },
    S#state{s = iface, iface = NewIface, signal = undefined};
xml_event(
    {endElement, _, "property", _},
    _L,
...
xml_event(endCDATA, _L, S) ->
    S;
xml_event(
    {startDTD, "node", "-//freedesktop//DTD D-BUS Object Introspection 1.0//EN", _SysID}, _L, S
) ->
    S;
...
end_method(#dbus_method{args = Args} = Method) ->
    ...
    Signature = lists:foldl(
        fun(#dbus_arg{type = Type}, Acc) ->
...
end_signal(#dbus_signal{args = Args} = Signal) ->
    OutArgs = lists:filter(
        fun
            (#dbus_arg{direction = out}) ->
                true;
            (#dbus_arg{direction = undefined}) ->
                true;
            (_) ->
...
build_annotation(
    [{_, _, "name", "org.freedesktop.DBus.Property.EmitsChangedSignal"} | Attrs], {undefined, V}
) ->
