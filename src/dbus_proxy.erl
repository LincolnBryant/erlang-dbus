...
%% @end
-spec call(
    Proxy :: dbus_proxy(), IfaceName :: dbus_name(), MethodName :: dbus_name(), Args :: term()
) ->
...
%% @end
-spec cast(
    Interface :: {interface, dbus_proxy(), dbus_name()}, MethodName :: dbus_name(), Args :: term()
) -> ok.
...
handle_call(
    {connect_signal, Name, '_', '_', Path, MFA},
    _From,
...
handle_call(
    {connect_signal, Name, IfaceName, SignalName, Path, MFA},
    _From,
    #state{handlers = Handlers} = State
) ->
    Match = [
...
handle_call(
    ...
    {reply, Ret, State};
handle_call(
    {connect_signal, IfaceName, SignalName, MFA},
    _From,
    #state{conn = {dbus_bus_connection, Conn}, path = Path, uniquename = Name} = State
) ->
...
handle_call(children, _From, #state{node = #dbus_node{name = Name, elements = Children}} = State) ->
    ...
    Paths = lists:map(
        fun(#dbus_node{name = ChildPath}) -> filename:join(Prefix, ChildPath) end, Children
    ),
    {reply, Paths, State};
...
handle_info(
    {dbus_signal, #dbus_message{header = Hdr, body = Args}}, #state{handlers = Handlers} = State
) ->
...
match_handler({signal_handler, '_', '_', '_', '_', _}, {_, _, _, _}) ->
    true;
match_handler({signal_handler, S, '_', '_', '_', _}, {S, _, _, _}) ->
    true;
match_handler({signal_handler, '_', I, '_', '_', _}, {_, I, _, _}) ->
    true;
match_handler({signal_handler, '_', '_', M, '_', _}, {_, _, M, _}) ->
    true;
match_handler({signal_handler, '_', '_', '_', PathMatch, _}, {_, _, _, P}) ->
    match_path(PathMatch, P);
match_handler({signal_handler, S, I, '_', '_', _}, {S, I, _, _}) ->
    true;
match_handler({signal_handler, S, '_', M, '_', _}, {S, _, M, _}) ->
    true;
match_handler({signal_handler, S, '_', '_', PathMatch, _}, {S, _, _, P}) ->
    match_path(PathMatch, P);
match_handler({signal_handler, S, I, M, '_', _}, {S, I, M, _}) ->
    true;
match_handler({signal_handler, S, I, '_', PathMatch, _}, {S, I, _, P}) ->
    match_path(PathMatch, P);
match_handler({signal_handler, S, I, M, PathMatch, _}, {S, I, M, P}) ->
    match_path(PathMatch, P);
match_handler(_, _) ->
    false.
...
do_handle_signal(
    #signal_handler{mfa = {Mod, Fun, Ctx}} = Handler, Acc, Sender, Iface, Signal, Path, Args
) ->
    case erlang:function_exported(Mod, Fun, 6) of
        true ->
            try
                Mod:Fun(Sender, Iface, Signal, Path, Args, Ctx)
            catch
                Cls:Err ->
...
do_handle_signal(
    #signal_handler{mfa = {Fun, Ctx}} = Handler, Acc, Sender, Iface, Signal, Path, Args
) ->
    try
        Fun(Sender, Iface, Signal, Path, Args, Ctx)
    catch
        Cls:Err ->
            ?error("Error dispatching signal to ~p/6: ~p:~p", [Fun, Cls, Err])
    end,
    [Handler | Acc];
do_handle_signal(#signal_handler{mfa = Pid} = Handler, Acc, Sender, Iface, Signal, Path, Args) when
    is_pid(Pid)
->
