...
-record(state, {
    ...
    waiting = [] :: list(),
    mechs = [] :: list(),
    %round       = 1       :: number(), UNUSED per hank
...
start_link(BusId, Options) when
    is_record(BusId, bus_id),
    is_list(Options)
->
...
start_link(BusId, ServiceReg, Options) when
    is_record(BusId, bus_id),
    is_list(Options)
->
...
init([#bus_id{scheme = tcp, options = BusOptions}, ServiceReg, Options, Owner]) ->
    {Host, Port} =
        case {lists:keysearch(host, 1, BusOptions), lists:keysearch(port, 1, BusOptions)} of
...
handle_event(
    {call, {_Pid, Tag} = From}, auth, connected, #state{sock = Sock, mechs = [Mech | Rest]} = State
) ->
    case Mech:init() of
        {ok, Resp} ->
        ...
        {continue, Resp, MechState} ->
            ...
            dbus_transport:send(Sock, <<"AUTH ", Resp/binary, "\r\n">>),
            gen_statem:reply(From, {ok, {self(), Tag}}),
            {next_state, waiting_for_data, State#state{
                waiting = [From], mechs = Rest, mech_state = {Mech, MechState}
            }}
    end;
...
handle_event({call, From}, {call, _}, waiting_for_reject, _State) ->
    ...
    keep_state_and_data;
handle_event(
    {call, {_Pid, Tag} = From}, auth, waiting_for_reject, #state{waiting = Waiting} = State
) ->
    gen_statem:reply(From, {ok, {self(), Tag}}),
    {keep_state, State#state{waiting = [From | Waiting]}};
%% STATE: waiting_for_agree
handle_event(
    info, {received, <<"AGREE_UNIX_FD\r\n">>}, waiting_for_agree, #state{sock = Sock} = State
) ->
...
handle_event(
    info, {received, <<"ERROR", Line/binary>>}, waiting_for_agree, #state{sock = Sock} = State
) ->
    case dbus_transport:support_unix_fd(Sock) of
        true ->
...
handle_event({call, From}, {call, _}, waiting_for_agree, _State) ->
    ...
    keep_state_and_data;
handle_event(
    {call, {_Pid, Tag} = From}, auth, waiting_for_agree, #state{waiting = Waiting} = State
) ->
    gen_statem:reply(From, {ok, {self(), Tag}}),
    {keep_state, State#state{waiting = [From | Waiting]}};
%% STATE: authenticated
...
process_ok(Data, #state{sock = Sock, waiting = Waiting} = State) ->
    ...
    case dbus_transport:support_unix_fd(Sock) of
        ...
        false ->
            ...
            {next_state, authenticated, State#state{
                waiting = [], guid = Guid, unix_fd = false, mech_state = undefined
            }}
    end.
...
record_mechs(Bin, #state{mechs = Mechs} = S) ->
    case parse_mech(Bin) of
        {unsupported, Name, Rest} ->
            ?debug("~s: Server offered mechanism \"~s\" that we don't know how to use~n", [
                S#state.side, Name
            ]),
            record_mechs(Rest, S);
        {Mech, Name, Rest} ->
            case lists:member(Mech, ?ALL_MECHANISMS) of
                true ->
                    ?debug("~s: Already tried mechanism ~s; not adding to list we will try~n", [
                        S#state.side, Name
                    ]),
                    record_mechs(Rest, S);
                false ->
