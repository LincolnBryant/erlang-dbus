...
signal(
    ...
    Path,
    Interface,
    #dbus_signal{name = SigName, out_sig = _Signature, out_types = Types},
    Args,
    Opts
) when
    is_list(Args)
...
-spec set_body(
    Method :: dbus_method(),
    Body :: term(),
    Message :: dbus_message()
) -> dbus_message() | {error, dbus_err()}.
...
set_body(
    Signature, Types, Body, #dbus_message{header = #dbus_header{fields = Fields} = Header} = Message
) ->
    try dbus_marshaller:marshal_list(Types, Body) of
        {Bin, Pos} ->
            ...
            Message#dbus_message{
                header = Header#dbus_header{fields = Fields2, size = Pos}, body = Bin
            }
    catch
        _:_ ->
...
match(HeaderMatches, #dbus_message{header = #dbus_header{fields = Fields}}) when
    is_list(HeaderMatches)
->
