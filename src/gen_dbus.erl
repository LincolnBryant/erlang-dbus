...
setup(DBus_config, State) ->
    ...
    {Iface, Interfaces} = lists:foldl(
        Fun,
        {undefined, dict:new()},
        DBus_config
    ),
    Node = #dbus_node{
        name = State1#state.path,
        interfaces = lists:map(
            fun({Key, {Methods, Signals}}) ->
                #dbus_iface{
                    name = Key,
                    methods = Methods,
                    signals = Signals
            ...
            ReplyMsg =
                case Reply of
                    ...
                    _ ->
                        ...
                        ReplyMsg1 = dbus_message:error(
                            Message, 'org.freedesktop.DBus.Error.Failed', "Failed"
                        ),
                        ReplyMsg1
                end,
...
do_signal(IfaceName, Signal, Args, _Options, Path = #state.path) ->
...
->
    lists:foldl(
        fun(Member, Interfaces1) ->
            member_build_introspect(
                MemberType,
                Member,
                State,
                Interfaces1
