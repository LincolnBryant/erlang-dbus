...
call_method(Config) ->
    {ok, O} = dbus_proxy:start_link(?config(bus, Config), ?SERVICE, <<"/root">>),
    ?assertMatch(
        {ok, [<<"Hello World">>, <<" from example-service.py">>]},
        dbus_proxy:call(O, ?IFACE, <<"HelloWorld">>, ["plop"])
    ),
    ?assertMatch(
        {error, {'org.freedesktop.DBus.InvalidParameters', _}},
        dbus_proxy:call(O, ?IFACE, <<"HelloWorld">>, [])
    ),
    ?assertMatch(
        {error, {'org.freedesktop.DBus.UnknownMethod', _}},
...
signal_all(Config) ->
    Fun = fun
        (_Sender, IfaceName, <<"SampleSignal">>, Path, _Args, Pid) ->
...
signal(Config) ->
    ...
    Wait = fun
        ...
        ([Signal | Signals], [], F) ->
            receive
                Signal -> F(lists:delete(Signal, Signals), [], F)
            after 100 -> ?assert(false)
            end;
        ...
        ([Signal | Signals], [BadSignal | BadSignals], F) ->
            receive
                Signal -> F(lists:delete(Signal, Signals), [BadSignal | BadSignals], F);
                BadSignal -> ?assert(false)
